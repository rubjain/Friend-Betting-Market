...
import { agoraAppUrl, agoraRealMoneyModeEnabled } from "./env.js";
import { getStripeClient, isStripeEnabled, stripePayoutsEnabled } from "./stripe.js";
...
export async function reviewDatabasePaymentTransaction({
  ...
  const result = await client.$transaction(async (tx) => {
    ...
    if (normalizedDecision === "approve" && transaction.type === "WITHDRAWAL") {
      const wantsStripePayout = transaction.provider === "stripe" || transaction.method === "stripe";
      if (wantsStripePayout && isStripeEnabled() && stripePayoutsEnabled()) {
        if (!agoraRealMoneyModeEnabled()) {
          return {
            ok: false,
            message: "Stripe payouts are disabled until real-money mode is enabled.",
        ...
        await tx.paymentTransaction.update({
          ...
          data: {
            ...
            metadata: {
              ...
              payoutId: payout.id,
              note: "Stripe payout created; awaiting payout.paid webhook.",
            },
          },
        });
        ...
        await tx.auditTrail.create({
          data: {
            actorId: reviewerId,
            action: "payment.withdrawal.payout.created",
            metadata: {
              paymentTransactionId: transaction.id,
              payoutId: payout.id,
              amount,
        ...
        return {
          ok: true,
          message: "Withdrawal approved; Stripe payout created and is processing.",
          userId: transaction.userId,
        };
      }
    }
...
export async function createDatabaseDeposit({ userId, amount, method } = {}) {
  ...
  if (fundingMethod === "stripe" && isStripeEnabled()) {
    if (!agoraRealMoneyModeEnabled()) {
      return {
        ok: false,
        message: "Stripe deposits are disabled until real-money mode is enabled.",
        code: "REAL_MONEY_DISABLED",
      };
    }
    const stripe = getStripeClient();
    const appUrl = agoraAppUrl();
    ...
    const transaction = await prisma.paymentTransaction.create({
      data: {
        ...
        provider: "stripe",
        metadata: {
          note: "Stripe Checkout session created; awaiting webhook completion.",
        },
      },
    });
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: { paymentTransactionId: transaction.id, userId },
      line_items: [
        {
          quantity: 1,
          price_data: {
            ...
            product_data: { name: "Agora deposit" },
          },
        },
      ],
    });
    ...
    return {
      ...
      transaction: { ...transaction, providerRef: session.id },
...
export async function requestDatabaseWithdrawal({ userId, amount, method } = {}) {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    const wantsStripe = fundingMethod === "stripe";
    if (wantsStripe && isStripeEnabled() && !agoraRealMoneyModeEnabled()) {
      return {
        ok: false,
        message: "Stripe withdrawals are disabled until real-money mode is enabled.",
        code: "REAL_MONEY_DISABLED",
      };
    }
    const transaction = await tx.paymentTransaction.create({
      data: {
        ...
        amount: withdrawalAmount,
        method: fundingMethod,
        provider: wantsStripe ? "stripe" : "demo-ledger",
        providerRef: wantsStripe ? null : `wd_${Date.now()}`,
        metadata: {
