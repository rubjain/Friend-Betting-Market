...
import { agoraAppUrl, agoraEmailFrom } from "./env.js";

const APP_URL = agoraAppUrl();
const FROM = agoraEmailFrom();
...
export function emailDeliveryConfigured(env = process.env) {
  return Boolean(env.EMAIL_USER && env.EMAIL_PASS && env.AGORA_EMAIL_FROM);
}
  ...
  await transporter.sendMail({
    ...
    html: `
      <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px 24px">
        ...
        <p style="color:#555;margin:0 0 24px">Click the button below to set a new password. This link expires in 30 minutes.</p>
        <a href="${link}" style="display:inline-block;padding:12px 24px;background:#e85d04;color:#fff;text-decoration:none;border-radius:8px;font-weight:600;font-size:15px">
          Reset password
        </a>
...
export async function sendAccountRecoveryEmail({ to, token }) {
  const link = `${APP_URL}/account-recovery?token=${token}`;

  if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS) {
    console.log(`[DEV] Account recovery link for ${to}: ${link}`);
    return;
  }
  ...
  await transporter.sendMail({
    ...
    html: `
      <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px 24px">
        ...
        <h2 style="font-size:20px;font-weight:700;color:#111;margin:0 0 8px">Recover your account</h2>
        <p style="color:#555;margin:0 0 24px">Use this link to set a new password and unlock your account. This link expires in 1 hour.</p>
        <a href="${link}" style="display:inline-block;padding:12px 24px;background:#e85d04;color:#fff;text-decoration:none;border-radius:8px;font-weight:600;font-size:15px">
          Recover account
        </a>
        <p style="color:#bbb;font-size:12px;margin:28px 0 0">If you didn't request this, you can ignore this email.</p>
