...
import { buildTradeExecution, normalizeTradingMode } from "../tradeExecution.js";
...
function getStore() {
  if (!globalThis.__agoraDemoStore) {
    globalThis.__agoraDemoStore = createDemoState();
  }
  sanitizeSportsState(globalThis.__agoraDemoStore);
  return globalThis.__agoraDemoStore;
}
...
export function placeDemoBet({ marketId, side, betDraft, isPaper = false }) {
  ...
  const result = calculatePayout({
    ...
    withdrawableShare: isPaper ? execution.grossAmount : betDraft.withdrawableShare,
    bonusShare: isPaper ? 0 : betDraft.bonusShare,
    market,
    adminConfig: state.adminConfig,
    oddsMultiplier: execution.grossAmount > 0 ? tradePreview.estimatedContracts / execution.grossAmount : 0,
  });
  ...
  if (isPaper) {
    addLedgerEntry(state, {
      ...
      metadata: `Placed ${side} paper bet on ${market.title} (fee ${execution.feeAmount.toFixed(2)} burned)`,
    });
  } else {
    addLedgerEntries(
      state,
    ...
    if (execution.feeAmount > 0) {
      addLedgerEntry(state, {
        user_id: state.currentUser.id,
        market_id: market.id,
        bet_id: betId,
        transaction_type: "credit",
        amount: execution.feeAmount,
        currency_type: "withdrawable",
        source: "platform_fee",
        metadata: `Trading fee recorded as platform revenue for ${market.title}`,
      });
    }
  }
  refreshDerivedBalances(state);
...
export function createDemoLimitOrder({ marketId, side, quantity, limitPrice, isPaper = false, limitExpiry }) {
  ...
  const execution = buildTradeExecution({
    tradingMode,
    netAmount: Number(quantity) * Number(limitPrice),
...
export function removeDemoFriend({ username, userId }) {
  const state = getStore();
  applySessionUser(state, userId);
  const normalized = normalizeUsername(username);
  const before = state.friends.list.length;
  state.friends.list = state.friends.list.filter((f) => {
    const record = friendRecordForUser(f, state.currentUser.id, state);
    return (record ?? f).username !== normalized;
  });
  if (state.friends.list.length === before) {
    return { ok: false, message: "Friend not found.", state: getDemoState(state.currentUser.id) };
  }
  return {
    ok: true,
    message: `Removed ${normalized} from your friends.`,
    state: getDemoState(state.currentUser.id),
  };
}
...
export function sellDemoBet({ betId, userId }) {
  const state = getStore();
  applySessionUser(state, userId);

  const idx = state.portfolio.openBets.findIndex((b) => b.id === betId);
  if (idx === -1) return { ok: false, message: "Bet not found.", state: getDemoState(state.currentUser.id) };

  const bet = state.portfolio.openBets[idx];
  if (bet.status !== "Open") return { ok: false, message: "Only open bets can be sold.", state: getDemoState(state.currentUser.id) };

  const oddsMultiplier = bet.oddsMultiplier ?? 2;
  const shares = (bet.stake ?? 0) * oddsMultiplier;

  // Look up current price from active markets
  ...
  if (bet.isPaper) {
    state.currentUser.paper_balance = (state.currentUser.paper_balance ?? 0) + proceeds;
  } else {
    state.currentUser.withdrawable_balance = (state.currentUser.withdrawable_balance ?? 0) + proceeds;
  }

  state.portfolio.openBets.splice(idx, 1);
  state.portfolio.pastBets.unshift({
    id: bet.id,
