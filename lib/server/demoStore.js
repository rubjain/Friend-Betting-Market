...
export function placeDemoBet({ marketId, side, betDraft }) {
...
export function sendDemoFriendInvite({ username }) {
  const state = getStore();
  ...
  if (normalized === state.currentUser.username.toLowerCase()) {
  ...
  const alreadyPending = state.friends.pending.some(
    (request) => request.username.toLowerCase() === normalized,
  );
...
export function handleDemoFriendRequest({ username, action }) {
  const state = getStore();
  const request = state.friends.pending.find((item) => item.username === username);

  if (!request) {
    return { ok: false, message: "Friend request was not found.", state: getDemoState() };
  }
  ...
  if (action === "accept" && request.direction === "incoming") {
    ...
    return {
      ok: true,
      message: `Accepted ${request.username} as a friend.`,
      state: getDemoState(),
    };
  }
  ...
  if (action === "decline") {
    return {
      ok: true,
      message: `Declined ${request.username}'s friend request.`,
      state: getDemoState(),
    };
  }
  ...
  return {
    ok: true,
    message: `Canceled the pending invite to ${request.username}.`,
    state: getDemoState(),
  };
}

export function toggleDemoFriendBoost({ username, marketId }) {
  const state = getStore();
  const friend = state.friends.list.find((item) => item.username === username);
  const market = getSelectedMarketFromState(state, marketId);

  if (!friend || !market) {
    return { ok: false, message: "Friend or market was not found.", state: getDemoState() };
  }
