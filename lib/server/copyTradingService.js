...
import { hasMarketplaceEntitlement } from "./marketplaceEntitlementService.js";
...
export function computeCopyStake(sourceStake, allocationPct, maxTradeSize = null) {
  const allocationStake = roundMoney(toNumber(sourceStake) * (toNumber(allocationPct) / 100));
  const cappedStake = maxTradeSize != null
    ? Math.min(allocationStake, toNumber(maxTradeSize))
    : allocationStake;
  return roundMoney(cappedStake);
}
...
async function validateCopy({ signal, subscription, client = prisma }) {
  ...
  const entitled = await hasMarketplaceEntitlement({
  ...
  if (subscription.maxOpenPositions != null) {
    const copiedOpen = await client.copyTrade.findMany({
      where: { subscriptionId: subscription.id, status: "COPIED", copiedBetId: { not: null } },
      select: { copiedBetId: true },
    });
    const openCount = copiedOpen.length
      ? await client.bet.count({
        where: { id: { in: copiedOpen.map((item) => item.copiedBetId) }, status: "OPEN" },
      })
      : 0;
