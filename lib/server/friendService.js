...
function relationStatus(friendship, userId) {
  if (!friendship) return "not_connected";
  if (friendship.status === "ACCEPTED") return "friends";
  if (friendship.status !== "PENDING") return "not_connected";
  return friendship.requesterId === userId ? "pending_outgoing" : "pending_incoming";
}
...
export async function searchDatabaseFriends({
  query,
  userId = defaultState.currentUser.id,
  client = prisma,
} = {}) {
  ...
  const usernameQuery = normalizeUsername(normalizedQuery);
  const users = await client.user.findMany({
    where: {
      id: { not: userId },
      OR: [
        { username: { contains: usernameQuery || normalizedQuery, mode: "insensitive" } },
...
export async function removeDatabaseFriend({
  username,
  userId = defaultState.currentUser.id,
  client = prisma,
}) {
  ...
  if (!other) {
    return { ok: false, message: "Friend not found.", state: await getDatabaseState(client, userId) };
  }
  ...
  if (!friendship) {
    return { ok: false, message: "Friendship not found.", state: await getDatabaseState(client, userId) };
  }

  await client.$transaction([
    client.friendship.delete({ where: { id: friendship.id } }),
    client.auditTrail.create({
      data: {
        actorId: userId,
