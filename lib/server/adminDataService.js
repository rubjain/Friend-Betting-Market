...
export function buildAdminAuditWhere({
  ...
  const where = {};
  const cleanedAction = cleanFilter(action);
  const cleanedActor = cleanFilter(actorId);
  const cleanedMarket = cleanFilter(marketId);
  const from = parseDateBoundary(dateFrom);
  const to = parseDateBoundary(dateTo, true);

  if (cleanedAction && cleanedAction !== "all") {
    where.action = cleanedAction;
  }
  if (cleanedActor) {
    where.actorId = cleanedActor;
  }
  if (cleanedMarket) {
    where.marketId = cleanedMarket;
  }
  if (from || to) {
    where.createdAt = {};
...
export async function getAdminAuditPage({
  ...
  await ensureDemoDatabaseSeed(prisma);
  const where = buildAdminAuditWhere({ action, actorId, marketId, dateFrom, dateTo });
