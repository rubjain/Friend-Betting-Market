...
async function syncMissingCatalogMarkets(client = prisma) {
  const existing = await client.market.findMany({ select: { id: true } });
  const ids = new Set(existing.map((m) => m.id));
  for (const market of defaultState.markets) {
    if (ids.has(market.id)) continue;
    await client.market.create({ data: buildMarketSeedCreateData(market) });
  }
}
