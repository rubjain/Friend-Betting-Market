import { calculatePayout, ammBuyYes, ammBuyNo } from "../marketMath.js";
...
export async function placeDatabaseBet({
  ...
  const outcome = await client.$transaction(async (tx) => {
    ...
    // ── AMM: compute real odds from the liquidity pool ───────────────────────
    function ammOddsForStake(stakeAmount) {
      const pool = market.liquidityPool;
      if (!pool || !(Number(pool.yesReserve) > 0) || !(Number(pool.noReserve) > 0)) return 2;
      const ammFn = side === "YES" ? ammBuyYes : ammBuyNo;
      const result = ammFn(
        Number(pool.yesReserve),
        Number(pool.noReserve),
        stakeAmount,
        Number(pool.feeBps) || 0,
      );
      return result ? result.oddsMultiplier : 2;
    }

    async function applyAmmReserveUpdate(tx, stakeAmount) {
      const pool = market.liquidityPool;
      if (!pool || !(Number(pool.yesReserve) > 0) || !(Number(pool.noReserve) > 0)) return;
      const ammFn = side === "YES" ? ammBuyYes : ammBuyNo;
      const executed = ammFn(
        Number(pool.yesReserve),
        Number(pool.noReserve),
        stakeAmount,
        Number(pool.feeBps) || 0,
      ...
      await tx.liquidityPool.update({
        where: { marketId },
        data: {
          yesReserve: executed.newYesReserve,
          noReserve: executed.newNoReserve,
          invariant: executed.newYesReserve * executed.newNoReserve,
        },
      });
      // Update market prices to reflect new pool state
      await tx.market.update({
        where: { id: marketId },
        data: { yesPrice: executed.newYesPrice, noPrice: executed.newNoPrice },
      });
      // Record price history snapshot so the chart has real data
      await tx.oddsSnapshot.create({
        data: {
          marketId,
          yesPrice: executed.newYesPrice,
          noPrice: executed.newNoPrice,
          source: "amm_trade",
        },
      });
    }
    ...
    if (isPaper) {
      ...
      const oddsMultiplier = ammOddsForStake(stake);
      ...
      await tx.ledgerEntry.create({
        data: {
          ...
          currency: "PAPER",
          source: "BET_PLACED",
          metadata: { note: `Paper bet: ${side} on "${market.title}" @ ${oddsMultiplier.toFixed(3)}x` },
        },
      });
      ...
      await applyAmmReserveUpdate(tx, result.totalStake);
      ...
      await tx.auditTrail.create({
        data: {
          ...
          metadata: { betId: bet.id, side, stake: result.totalStake, oddsMultiplier },
        },
      });
    ...
    const oddsMultiplier = ammOddsForStake(Number(betDraft.stake) || 0);
    const result = calculatePayout({
