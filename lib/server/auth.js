...
export async function loginUser({ identifier }) {
  if (!hasDatabaseUrl()) {
    ...
    return { ok: true, session: fallback, user: defaultState.currentUser };
  }
...
export async function signupUser({ name, email, username }) {
  if (!hasDatabaseUrl()) {
    return {
      ok: false,
      message: "Connect DATABASE_URL to create real accounts. Demo mode can still log in locally.",
    };
  }
  ...
  return loginUser({ identifier: cleanedEmail });
}
