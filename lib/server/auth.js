...
export async function signupUser({ name, email, username, password, rateLimitKey }) {
  ...
  try {
    ...
    const login = await loginUser({ identifier: cleanedEmail, password, rateLimitKey });
    return {
      ...login,
      emailVerificationToken: verification.token,
      message: "Account created. Verify your email before using production-money features.",
    };
  } catch {
    await recordRateLimitFailure(signupLimit.key, RATE_LIMITS.signup);
