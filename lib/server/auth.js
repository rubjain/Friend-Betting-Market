...
import { sendVerificationEmail, sendPasswordResetEmail, sendAccountRecoveryEmail } from "./email.js";
import {
  RATE_LIMITS,
  checkRateLimit,
  clearRateLimit,
...
import { authenticateApiKey, hasScope } from "./apiKeyService.js";
import { hasAdminPermission } from "./adminPermissions.js";
import { agoraDevAdminShortcutEnabled } from "./env.js";
...
function allowDevAdminShortcut() {
  return agoraDevAdminShortcutEnabled();
}
...
export async function getSessionFromRequest(request) {
  ...
  const headerAdmin =
    allowDevAdminShortcut() && request.headers.get("x-agora-role") === "admin";
...
/**
 * Identify the caller for /api/v1/* routes: Bearer API key or signed session cookie.
 * Applies per-key or per-session+IP rate limits.
 */
export async function resolvePublicApiCaller(request) {
  const bearer = parseBearerToken(request);
  const ip = clientIpFromRequest(request);

  if (bearer) {
    const keyFingerprint = crypto.createHash("sha256").update(bearer).digest("hex").slice(0, 24);
    const rl = await checkRateLimit({
      scope: "publicApiKey",
      identifier: keyFingerprint,
      requestKey: ip,
      config: RATE_LIMITS.publicApiKey,
    });
    if (!rl.ok) {
      return {
        ok: false,
        response: NextResponse.json(
          { ok: false, message: rateLimitMessage(rl), code: "RATE_LIMITED" },
          {
            status: 429,
    ...
    const keyAuth = await authenticateApiKey(bearer);
    if (!keyAuth.ok) {
      return {
        ok: false,
        response: NextResponse.json(
          {
            ok: false,
            message: keyAuth.message || "Invalid API key.",
    ...
    return { ok: true, userId: keyAuth.userId, apiKeyScopes: keyAuth.scopes || [], auth: "apiKey" };
  }
  ...
  const rl = await checkRateLimit({
    scope: "publicApiSession",
    identifier: session.userId,
    requestKey: ip,
    config: RATE_LIMITS.publicApiSession,
  });
  if (!rl.ok) {
    return {
      ok: false,
      response: NextResponse.json(
        { ok: false, message: rateLimitMessage(rl) },
        {
          status: 429,
          headers: { "Retry-After": String(rl.retryAfterSeconds ?? 60) },
        },
  ...
  return { ok: true, userId: session.userId, apiKeyScopes: null, auth: "session", session };
}

export function requireApiKeyScopes(apiKeyScopes, requiredScope) {
  if (!requiredScope || apiKeyScopes == null) {
    return { ok: true };
  }
  if (!hasScope(apiKeyScopes, requiredScope)) {
    return {
      ...
      response: NextResponse.json(
        {
          ok: false,
          message: `API key missing scope ${requiredScope}.`,
          code: "INVALID_SCOPE",
        },
        { status: 403 },
      ),
...
export async function verifyEmailCode({ email, code, rateLimitKey }) {
  ...
  const limit = await checkRateLimit({
    ...
    identifier: cleaned,
    requestKey: rateLimitKey,
    config: RATE_LIMITS.emailVerification,
  });
  if (!limit.ok) {
    return { ok: false, message: rateLimitMessage(limit) };
  }
  ...
  if (!user) {
    await recordRateLimitFailure(limit.key, RATE_LIMITS.emailVerification);
...
export async function changePassword({ userId, currentPassword, newPassword, rateLimitKey }) {
  if (!hasDatabaseUrl()) {
    return { ok: false, message: "Password change requires a database connection." };
  }
  ...
  const limit = await checkRateLimit({
    ...
    identifier: userId,
    requestKey: rateLimitKey,
    config: RATE_LIMITS.changePassword,
  });
  if (!limit.ok) {
    return { ok: false, message: rateLimitMessage(limit) };
  }
  ...
  if (!currentPassword) {
    await recordRateLimitFailure(limit.key, RATE_LIMITS.changePassword);
  ...
  if (currentPassword === newPassword) {
    return { ok: false, message: "Choose a new password that is different from your current one." };
  }

  const user = await prisma.user.findUnique({ where: { id: userId } });
  if (!user?.passwordHash) {
    await recordRateLimitFailure(limit.key, RATE_LIMITS.changePassword);
    return { ok: false, message: "This account needs a password reset before you can change it here." };
  }
...
export async function requestAccountRecovery({ identifier, rateLimitKey }) {
  ...
  sendAccountRecoveryEmail({ to: user.email, token: recovery.token }).catch(() => {});
...
export async function logoutUser(session) {
  if (hasDatabaseUrl() && session?.sessionId) {
    await prisma.userSession.delete({ where: { id: session.sessionId } });
  }
}
