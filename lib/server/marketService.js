...
export async function approveDatabaseMarket({
  ...
  await client.$transaction([
    ...
    // Seed the LMSR market maker at 50/50.
...
export async function resolveDatabaseMarket({
  ...
  const outcome = await client.$transaction(async (tx) => {
    ...
    for (const bet of bets) {
      ...
      if (bet.isPaper) {
        ...
        if (normalizedResult === "VOID") {
          ...
          await tx.bet.update({ where: { id: bet.id }, data: { status: "VOIDED", settledAt: new Date(), realizedPnL: 0, unrealizedPnL: 0 } });
          continue;
        }
        ...
        if (bet.side === normalizedResult) {
          ...
          await tx.bet.update({ where: { id: bet.id }, data: { status: "WON", settledAt: new Date(), realizedPnL: settlement.boostedPayout - stakeAmount, unrealizedPnL: 0 } });
        } else {
          await tx.bet.update({ where: { id: bet.id }, data: { status: "LOST", settledAt: new Date(), realizedPnL: -stakeAmount, unrealizedPnL: 0 } });
        }
        continue;
      }
      ...
      if (bet.side === normalizedResult) {
        // Use the real market-maker shares stored at bet placement time.
        // Old bets have expectedMultiplier around 1.0 (social boost only);
        // treat any value below 1.3 as legacy and fall back to 2x.
        const storedMultiplier = Number(bet.expectedMultiplier) || 0;
        const oddsMultiplier = storedMultiplier >= 1.3 ? storedMultiplier : 2;
        const settlement = calculatePayout({
        ...
        await tx.bet.update({ where: { id: bet.id }, data: { status: "WON", settledAt: new Date(), realizedPnL: settlement.boostedPayout - betForLedger.stake, unrealizedPnL: 0 } });
      } else {
        await tx.bet.update({ where: { id: bet.id }, data: { status: "LOST", settledAt: new Date(), realizedPnL: -betForLedger.stake, unrealizedPnL: 0 } });
      }
    }
    ...
    return {
      ok: true,
      message: `Resolved "${market.title}" as ${normalizedResult}.`,
      copiedProfileIds: [
        ...new Set(
          bets
            .map((bet) => bet.copiedFromProfileId)
            .filter(Boolean),
        ),
      ],
    };
  ...
  if (outcome.ok && outcome.copiedProfileIds?.length) {
    await Promise.all(
      outcome.copiedProfileIds.map((profileId) => recomputeProfileStats(profileId, client).catch(() => null)),
    );
  }
