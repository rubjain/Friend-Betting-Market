...
import { createAuthToken } from "./authSecurity.js";
import { createEmailVerificationCode } from "./auth.js";
import { sendVerificationEmail } from "./email.js";

const verificationTypes = new Set(["email", "phone", "identity", "payment", "device", "location", "age", "sanctions"]);
...
export async function updateDatabaseProfile({
  ...
  try {
    ...
    emailChanged = Boolean(existing && existing.email !== cleanedEmail);
    ...
    await client.$transaction(async (tx) => {
      ...
      if (emailChanged) {
        await tx.verificationCheck.upsert({
          ...
          update: { status: "PENDING", provider: "email-change", metadata: { changedAt: new Date().toISOString() } },
          create: { userId, type: "EMAIL", status: "PENDING", provider: "email-change" },
        });
      ...
      await tx.auditTrail.create({
        data: {
          ...
          metadata: { name: cleanedName, email: cleanedEmail, emailChanged: Boolean(emailChanged) },
        },
      });
    });
  } catch {
