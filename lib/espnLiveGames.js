...
import { agoraDisableEspn } from "./server/env.js";
...
function scoreboardDateKeys(now = new Date(), lookAheadDays = 1) {
  const tz = "America/New_York";
  const keys = new Set();
  // Always include yesterday for recently-completed games
  keys.add(yyyymmddInTimeZone(new Date(now.getTime() - 86400000), tz));
  // Today through lookAheadDays
  for (let i = 0; i <= lookAheadDays; i++) {
    keys.add(yyyymmddInTimeZone(new Date(now.getTime() + i * 86400000), tz));
  }
  return [...keys];
}
...
/**
 * Extract moneyline American-odds integers from ESPN's competition.odds array.
 * Returns { home, away } (integers like -155 / +130) or null if unavailable.
 * ESPN carries DraftKings or ESPN BET lines for most US sport events.
 */
function extractEspnMoneyline(competition) {
  const entry = competition?.odds?.[0];
  const ml = entry?.moneyline;
  if (!ml) return null;
  // Prefer closing line, fall back to opening line
  const homeStr = ml.home?.close?.odds ?? ml.home?.open?.odds;
  const awayStr = ml.away?.close?.odds ?? ml.away?.open?.odds;
...
function mapNbaEvent(event, now) {
  ...
  const game = {
    ...
    moneyline: extractEspnMoneyline(competition),
  };
...
function mapTennisCompetition(parentEvent, competition, now, leagueTag, boardSlug, bracketLabel) {
  ...
  // Skip bracket slots where players haven't been determined yet (TBD/TBA/empty)
  const isTbd = (n) => !n || /^\s*(tbd|tba)\s*$/i.test(n);
  if (isTbd(homeName) || isTbd(awayName)) {
    return null;
  }
  ...
  return {
    ...
    moneyline: extractEspnMoneyline(competition), // rarely present for tennis
  };
}
...
/**
 * Core ESPN fetch for a given set of dates. Shared by live games and market sync.
 */
async function fetchEspnGamesForDates(dates, options = {}) {
  ...
  for (const event of raw) {
    ...
    let mapped = null;
    if (uid.includes("~l:46~")) mapped = mapNbaEvent(event, new Date());
    else if (uid.includes("~l:10~")) mapped = mapMlbEvent(event, new Date());
    else if (uid.includes("~l:28~")) mapped = mapNflEvent(event, new Date());
    else if (uid.includes("~l:90~")) mapped = mapNhlEvent(event, new Date());
  ...
  SOCCER_LEAGUES.forEach(([slug, tag], i) => {
    ...
    for (const event of batch) {
      const mapped = mapSoccerEvent(event, new Date(), tag, slug);
      if (mapped) games.push(mapped);
    }
  });

  games.push(...flattenTennisScoreboardEvents(atpEvents, new Date(), "ATP", "atp"));
  games.push(...flattenTennisScoreboardEvents(wtaEvents, new Date(), "WTA", "wta"));

  return sortGames(games);
}

/**
 * Fetch upcoming games for market sync — covers today + next 6 days.
 * Wider window than the live score rail so we seed markets before game day.
 */
export async function fetchEspnGamesForSync(options = {}) {
  if (agoraDisableEspn()) return [];
  const dates = scoreboardDateKeys(new Date(), 6);
  return fetchEspnGamesForDates(dates, options);
}

/**
 * Returns normalized games for the live score rail (yesterday / today / tomorrow).
 */
export async function fetchEspnLiveGames(now = new Date(), options = {}) {
  if (agoraDisableEspn()) return [];
  const dates = scoreboardDateKeys(now, 1);
  return fetchEspnGamesForDates(dates, options);
}
