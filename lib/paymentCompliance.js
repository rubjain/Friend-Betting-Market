import { agoraRealMoneyModeEnabled } from "./server/env.js";
...
const REQUIRED_VERIFICATIONS = {
  ...
  /** When AGORA_REAL_MONEY_MODE=1, real (non-paper) bets require these checks */
...
function numberSetting(user, key) {
  if (key === "dailyDepositLimit" && user?.dailyDepositLimit != null) {
    return Number(user.dailyDepositLimit);
  }
  const value = Number(user?.settings?.[key]);
  return Number.isFinite(value) ? value : null;
}

function selfExcludedUntil(user, now = Date.now()) {
  const direct = user?.selfExcludedUntil;
  if (direct) return direct instanceof Date ? direct.toISOString() : String(direct);
  return user?.settings?.selfExcludedUntil ?? null;
}
...
export function evaluatePaymentCompliance({ user, action, amount, env = process.env, now = Date.now() } = {}) {
  ...
  if (isFutureDate(selfExcludedUntil(user), now)) {
